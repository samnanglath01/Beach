...
class DatabaseHelper {
    ...
    class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
        ...
        //This was added for CalMenuFragment
        fun getCalMenuItems(day: String, diningHall: String): List<String> {
            val db = this.readableDatabase
            val menuItems = HashSet<String>() // Use HashSet to store distinct items
            val selection = "$COLUMN_DAY = ? AND $COLUMN_DINING_HALL = ?"
            val selectionArgs = arrayOf(day, diningHall)
            val cursor = db.query(
                TABLE_NAME,
                arrayOf(COLUMN_MENU_ITEM), // Only need the menu item column.
                selection,
            ...
            return menuItems.toList() // Convert HashSet to List to maintain distinctness
        }
